...
{
    public function list(VendorAdmin $vendorAdmin): LengthAwarePaginator
    {
        return SupportTicket::where('requester_user_id', $vendorAdmin->vendor_id)
            ->where('requester_role', 'seller')
            ->orderByDesc('created_at')
            ->paginate(20);
    }

    public function store(VendorAdmin $vendorAdmin, array $data): SupportTicket
    {
        $ticket = SupportTicket::create([
            'ticket_number'     => $this->generateTicketNumber(),
            'requester_user_id' => $vendorAdmin->vendor_id,
    ...
    public function findForVendor(VendorAdmin $vendorAdmin, string $ticketNumber): ?SupportTicket
    {
    ...
    public function addMessage(VendorAdmin $vendorAdmin, SupportTicket $ticket, array $data): TicketMessage
    {
        $message = $ticket->messages()->create([
            'sender_type'      => VendorAdmin::class,
            'sender_id'        => $vendorAdmin->id,
