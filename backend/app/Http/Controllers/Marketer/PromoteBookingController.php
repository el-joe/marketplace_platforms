...
class PromoteBookingController extends Controller
{
    use HasDataTable;
    ...
    {
        return PaidAdBooking::where('advertiser_type', 'marketer')
    ...
    {
        $marketerId = $this->marketer()->id;

        $counts = PaidAdBooking::where('advertiser_type', 'marketer')->where('marketer_id', $marketerId)
            ->selectRaw('status, count(*) as c')->groupBy('status')->pluck('c', 'status');

        return view('marketer.promote.bookings.index', compact('counts'));
    }
    ...
    public function datatable(Request $request): JsonResponse
    {
        ...
        $query = PaidAdBooking::where('advertiser_type', 'marketer')->where('marketer_id', $marketerId)
        ...
        return $this->dataTableResponse($request, $query, $columns, fn (PaidAdBooking $b) => [
            'reference' => '<a href="'.route('marketer.promote.bookings.show', $b->id).'" class="font-medium text-primary-600 hover:underline">'.e($b->booking_reference).'</a>',
            'slot' => e($b->slot?->name),
            ...
            'actions' => '<a href="'.route('marketer.promote.bookings.show', $b->id).'" class="inline-flex items-center px-3 py-1.5 text-xs font-medium border border-gray-200 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors">View</a>',
        ]);
    }
    ...
    public function show(string $booking): View
    {
        ...
        $attribution = app(\App\Services\Ads\AdAttributionService::class)->forBooking($booking);

        return view('marketer.promote.bookings.show', compact('booking', 'stats', 'attribution'));
    }
    ...
    public function uploadCreative(Request $request, string $booking): JsonResponse
    {
        ...
        $data = $request->validate([
            ...
            'destination_type' => ['required', 'in:marketer_profile,campaign'],
        ...
        try {
            $this->creativeService->upload($booking, $data, $files, Auth::guard('marketer')->user());
        } catch (DomainException $e) {
            return response()->json(['success' => false, 'message' => $e->getMessage()], 422);
        }
    ...
    public function pay(string $booking): JsonResponse
    {
        ...
        try {
            ...
            $wallet = app(WalletService::class)->getOrCreateWallet('marketer', $booking->marketer_id, $booking->currency);
    ...
    public function cancel(Request $request, string $booking): JsonResponse
    {
        ...
        try {
            $this->bookingService->cancel($booking, 'marketer', $data['reason']);
        } catch (DomainException $e) {
            return response()->json(['success' => false, 'message' => $e->getMessage()], 422);
        }
