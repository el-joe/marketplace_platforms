...
use App\Http\Requests\Marketer\Ads\CancelBookingRequest;
use App\Http\Requests\Marketer\Ads\CreateBookingRequest;
use App\Http\Requests\Marketer\Ads\UploadCreativeRequest;
use App\Http\Resources\Vendor\Ads\AdBookingResource;
use App\Http\Resources\Vendor\Ads\CreativeResource;
...
class AdBookingController extends Controller
{
    ...
    {
        $query = PaidAdBooking::where('advertiser_type', 'marketer')
            ->where('marketer_id', $this->marketer()->id)
            ->with(['slot.placementDefinition', 'slot.pageBlock.page', 'slot.country', 'creatives.files'])
            ->latest();
    ...
    public function uploadCreative(UploadCreativeRequest $request, string $id): JsonResponse
    {
        ...
        try {
            $creative = $this->creativeService->upload($booking, $request->validated(), $files, auth('marketer_api')->user());
        } catch (DomainException $e) {
            return ApiResponse::error($e->getMessage());
        }

        return ApiResponse::success(new CreativeResource($creative), 'Success', 201);
    }
    ...
    public function cancel(CancelBookingRequest $request, string $id): JsonResponse
    {
        ...
        try {
            $this->bookingService->cancel($booking, 'marketer', $request->validated('reason'));
        } catch (DomainException $e) {
            return ApiResponse::error($e->getMessage());
        }
