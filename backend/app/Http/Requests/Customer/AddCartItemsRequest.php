...
class AddCartItemsRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'items.*.listing_type'           => ['nullable', 'string', 'in:vendor,admin,marketer'],
            'items.*.vendor_listing_id'      => ['required_if:items.*.listing_type,vendor', 'required_unless:items.*.listing_type,admin,marketer', 'nullable', 'uuid', 'exists:vendor_listings,id'],
            'items.*.admin_listing_id' => ['required_if:items.*.listing_type,admin', 'nullable', 'uuid', 'exists:admin_listings,id'],
            'items.*.marketer_listing_id' => ['required_if:items.*.listing_type,marketer', 'nullable', 'uuid', 'exists:marketer_listings,id'],
