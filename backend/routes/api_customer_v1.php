...
use App\Http\Controllers\Api\Customer\BookableUnitAvailabilityController;
...
use App\Http\Controllers\Customer\Api\TravelPackageInquiryController;
use App\Http\Controllers\Customer\AuthController;
use App\Http\Controllers\Customer\BannerClickController;
...
use App\Http\Controllers\Customer\HelpCenterController;
...
Route::get('currencies', [CurrencyController::class, 'index'])->name('customer.currencies.index');
...
Route::prefix('products')->name('customer.products.')->group(function (): void {
    ...
    Route::get('/{typeAndId}', [ListingDetailController::class, 'showByTypeId'])
...
Route::post(
    ...
    [TravelPackageInquiryController::class, 'store']
...
Route::get('travel/{id}', [ListingController::class, 'show'])
    ->whereUuid('id')
    ->defaults('type', 'travel')
    ->name('customer.travel.show');

Route::get('classified/{id}', [ListingController::class, 'show'])
    ->whereUuid('id')
...
Route::get(
    'categories/{slug}',
    function (Request $request, $country, string $slug) {
...
Route::prefix('help-center')->name('customer.help-center.')->group(function (): void {
    Route::get('tree', [HelpCenterController::class, 'tree'])->name('tree');
});
...
Route::post('banners/{banner}/click', [BannerClickController::class, 'click'])
...
Route::prefix('marketers/{marketer}/contract')->name('customer.api.marketer-contract.')->group(function (): void {
    Route::get('/', [MarketerContractController::class, 'show'])->name('show');
    Route::get('download', [MarketerContractController::class, 'downloadActivePdf'])
        ->middleware('auth:customer')
        ->name('download');
    Route::post('accept', [MarketerContractController::class, 'accept'])
        ->middleware('auth:customer')
        ->name('accept');
});
...
Route::middleware('auth:customer')->group(function (): void {
    ...
    Route::prefix('special-requests')->name('customer.special-requests.')->group(function (): void {
        ...
        Route::patch('{id}/close', [SpecialRequestController::class, 'close'])->name('close');
    });
