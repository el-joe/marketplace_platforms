...
function initSurchargesTable() {
    const routes = window.WAREHOUSE_SURCHARGE_ROUTES;

    window.initDataTable('warehouse-shipping-surcharges-table', {
        url: routes.index,
        ajaxMethod: 'GET',
        order: [[0, 'asc']],
        columns: [
...
function initSurchargeModal() {
    const $modal = $('#warehouse-surcharge-modal');
    const $form = $('#warehouse-surcharge-form');
...
$(function () {
    if (!document.getElementById('warehouse-shipping-surcharges-table')) return;
    initSurchargesTable();
    initSurchargeModal();
});
