{{--
    Role form partial for the Partner panel.
    Include with: @include('partner.roles._form', ['mode' => 'create'])
...
<div class="px-4 py-6 sm:px-6 lg:px-8"
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">
                {{ $isEdit ? __('partner.roles.edit_role_title', ['name' => $role->name]) : __('partner.roles.new_role') }}
            </h1>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('partner.roles.index') }}"
            ...
            <button type="submit" id="save-btn"
                ...
                {{ $isEdit ? __('partner.roles.save_changes') : __('partner.roles.create_role') }}
            </button>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm mb-4">
        <div class="px-5 py-4 border-b border-gray-100">
            <h2 class="text-sm font-semibold text-gray-900">{{ __('partner.roles.role_details') }}</h2>
        </div>
        <div class="px-5 py-5">
            <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('partner.roles.role_name') }} <span class="text-red-500">*</span></label>
            <input type="text" name="name" autocomplete="off" value="{{ old('name', $isEdit ? $role->name : '') }}"
                pattern="[a-z0-9_]+" placeholder="{{ __('partner.roles.role_name_placeholder') }}"
            ...
            <p class="mt-1 text-xs text-gray-500">{{ __('partner.roles.role_name_help') }}</p>
        </div>
    </div>

    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 class="text-sm font-semibold text-gray-900">{{ __('partner.roles.permissions') }}</h2>
            <span class="text-xs text-gray-500 bg-gray-100 rounded-full px-2 py-0.5" x-text="selectedPermissions.length + ' {{ __('partner.roles.selected') }}'"></span>
        </div>
