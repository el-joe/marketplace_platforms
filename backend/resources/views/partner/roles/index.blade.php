...
@section('title', __('partner.roles.page_title'))
@section('page-title', __('partner.roles.title'))
...
@push('scripts')
    ...
    <script>
        window.ROLES = {
            csrf: '{{ csrf_token() }}',
            destroyUrl: '{{ route('partner.roles.destroy', ':id') }}',
        };
    </script>
@endpush
...
@section('content')
    <div class="px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">{{ __('partner.roles.manage_roles') }}</h1>
                <p class="mt-1 text-sm text-gray-500">{{ __('partner.roles.manage_roles_subtitle') }}</p>
            </div>
            <a href="{{ route('partner.roles.create') }}"
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('partner.roles.role_name') }}</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('partner.roles.permissions_column') }}</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('partner.roles.members_column') }}</th>
                            ...
                            <td class="px-6 py-4">
                                ...
                                @if ($isProtected)
                                    <span class="ms-2 inline-flex items-center rounded-full bg-purple-100 text-purple-700 px-2 py-0.5 text-xs font-medium">
                                        {{ __('partner.roles.default_badge') }}
                                    </span>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4 text-left">
                                @unless ($isProtected)
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('partner.roles.edit', $role->id) }}"
