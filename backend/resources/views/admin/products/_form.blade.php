        ...
        <div class="w-full">
            ...
            <div class="bg-white rounded-t-xl border border-gray-200 overflow-hidden">
                <nav class="flex overflow-x-auto border-b border-gray-100" aria-label="{{ __('admin.products.form_tabs_aria') }}">
                    ...
                    <button
                        type="button"
                        @click="activeTab = 'promo_badges'"
                        :class="activeTab === 'promo_badges'
                            ? 'border-b-2 border-primary-600 text-primary-700 bg-primary-50/50'
                            : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'"
                        class="flex items-center gap-1.5 px-3 sm:px-4 py-3 sm:py-3.5 text-sm font-medium -mb-px whitespace-nowrap transition-colors"
                    >
                        <x-heroicon name="tag" class="w-4 h-4" />
                        {{ __('admin.products.tab_promo_badges') }}
                    </button>
                    @endif
            ...
            {{-- ─────────────────────────────────────────────── --}}
            <div
                x-show="activeTab === 'promo_badges'"
                x-cloak
                x-data="{
                    badges: @js(collect($promoBadges ?? [])->map(fn($b) => [
                    ...
                    add() { if (this.badges.length < 10) this.badges.push({ id: '', label_en: '', label_ar: '', icon_key: 'Truck', color_hex: '#1a1a2e', text_color_hex: '#FFFFFF', is_active: true }); },
                }"
                class="bg-white rounded-b-xl border border-t-0 border-gray-200 p-4 sm:p-6 shadow-sm space-y-4"
            >
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-sm font-semibold text-gray-800">{{ __('admin.products.tab_promo_badges') }}</h3>
                        <p class="text-xs text-gray-500">Messages rotated on the product card and product page, in the order shown. Max 10.</p>
                    </div>
                    <button type="button" @click="add()" class="text-sm font-medium text-primary-600 hover:text-primary-700">+ Add badge</button>
                </div>

                <datalist id="promo-badge-icons">
                    @foreach(['Truck','Car','Zap','Tag','Percent','Gift','Star','Flame','ShieldCheck','Clock','BadgeCheck','Sparkles','Rocket','Package'] as $ic)
                    <option value="{{ $ic }}">
                    @endforeach
                </datalist>

                <p x-show="badges.length === 0" class="text-sm text-gray-400 italic">No badges yet.</p>
                ...
                <template x-for="(b, i) in badges" :key="i">
                    <div class="rounded-lg border border-gray-200 p-3 grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
                        ...
                        <div class="md:col-span-2">
                            <label class="block text-xs text-gray-500 mb-1">Label (AR)</label>
                        ...
                        <div>
                            <label class="block text-xs text-gray-500 mb-1">Icon (Lucide name)</label>
                        ...
                        <div class="md:col-span-6 flex items-center justify-between">
                            <label class="flex items-center gap-2 text-sm text-gray-600">
                                ...
                                <input type="checkbox" :name="`promo_badges[${i}][is_active]`" value="1" x-model="b.is_active"> Active
                            </label>
                            <div class="flex gap-3 text-sm">
                                <button type="button" x-show="i > 0" @click="badges.splice(i-1, 2, badges[i], badges[i-1])" class="text-gray-500 hover:text-gray-800">↑ Up</button>
                                <button type="button" @click="badges.splice(i, 1)" class="text-red-500 hover:text-red-700">Remove</button>
                            </div>
                        </div>
                    </div>
                </template>
