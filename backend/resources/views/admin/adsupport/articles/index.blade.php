...
@section('title', __('admin.adsupport.articles'))
...
<div class="p-6 space-y-5">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('admin.adsupport.articles') }}</h1>
            <p class="text-sm text-gray-500 mt-0.5">{{ __('admin.adsupport.manage_all_content') }}</p>
        </div>
        <a href="{{ route('admin.adsupport.articles.create') }}"
           class="inline-flex items-center gap-2 rounded-lg bg-primary-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-primary-700">
            ...
            {{ __('admin.adsupport.new_article') }}
        </a>
    </div>

    {{-- Stats --}}
    <div class="grid grid-cols-3 gap-4">
        <x-stat-card title="{{ __('admin.adsupport.published_articles') }}" :value="number_format($stats['published'])" icon="check-circle" iconBg="bg-emerald-100 text-emerald-600" />
        <x-stat-card title="{{ __('admin.adsupport.draft_articles') }}" :value="number_format($stats['draft'])" icon="pencil" iconBg="bg-gray-100 text-gray-600" />
        <x-stat-card title="{{ __('admin.adsupport.total_views') }}" :value="number_format($stats['total_views'])" icon="eye" iconBg="bg-primary-100 text-primary-600" />
    </div>
    ...
    <x-card>
        <form id="articles-filter-form" class="flex flex-wrap gap-3 items-end">
            <div class="flex-1 min-w-[160px]">
                ...
                <input type="text" id="search-input" class="form-input w-full text-sm" placeholder="{{ __('admin.adsupport.search_placeholder') }}">
            </div>
            <div class="w-40">
                <label class="block text-xs font-medium text-gray-600 mb-1">{{ __('common.status') }}</label>
                <select id="filter-status" class="form-input w-full text-sm">
                    <option value="">{{ __('admin.adsupport.all_statuses') }}</option>
            ...
            <div class="w-56">
                ...
                <select id="filter-collection" class="form-input w-full text-sm">
                    <option value="">{{ __('admin.adsupport.all_collections') }}</option>
            ...
            <table id="articles-table" class="w-full text-sm" style="width:100%">
                <thead>
                    <tr class="border-b border-gray-100 text-start">
                        <th class="py-2 pr-4 text-xs font-medium text-gray-500 uppercase">{{ __('admin.adsupport.article_title') }}</th>
                        ...
                        <th class="py-2 pr-4 text-xs font-medium text-gray-500 uppercase">{{ __('admin.adsupport.views') }}</th>
...
<x-modal id="delete-article-modal" title="{{ __('admin.adsupport.delete_article') }}" size="sm">
    <p class="text-sm text-gray-700">{{ __('admin.adsupport.soft_delete_article_confirm') }}</p>
...
(function () {
    ...
    function getFilters() {
        return {
            ...
            ad_support_collection_id: document.getElementById('filter-collection').value,
        };
    }
    ...
    function initTable() {
        if (typeof window.initDataTable !== 'function') {
            dtInstance = window.$('#articles-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{{ route("admin.adsupport.articles.datatable") }}',
                    type: 'POST',
                    headers: { 'X-CSRF-TOKEN': CSRF },
                    data: function (d) {
                        const f = getFilters();
                        d.status = f.status;
                        d.ad_support_collection_id = f.ad_support_collection_id;
                        if (f.search.value) d.search = f.search;
                    },
                },
                columns: [
                    { data: 'title', orderable: true },
                    { data: 'collection', orderable: false },
    ...
    ['filter-status', 'filter-collection'].forEach(id => {
    ...
    document.getElementById('clear-filters')?.addEventListener('click', () => {
        ['filter-status', 'filter-collection', 'search-input'].forEach(id => {
