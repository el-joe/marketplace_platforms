...
<div class="p-6">
    ...
    <form id="article-form" method="POST" action="{{ route('admin.adsupport.articles.update', $article->id) }}">
        @csrf
        @method('PUT')
                            ...
                            <select name="ad_support_collection_id" class="form-input w-full text-sm @error('ad_support_collection_id') is-invalid @enderror">
                                ...
                                @foreach($collections as $parent)
                                    <optgroup label="{{ $parent->name }}">
                                        <option value="{{ $parent->id }}" {{ old('ad_support_collection_id', $article->ad_support_collection_id) == $parent->id ? 'selected' : '' }}>
                                            {{ $parent->name }}
                                        </option>
                                        @foreach($parent->children as $child)
                                            <option value="{{ $child->id }}" {{ old('ad_support_collection_id', $article->ad_support_collection_id) == $child->id ? 'selected' : '' }}>
                                                ↳ {{ $child->name }}
                                            </option>
                                        @endforeach
                                    </optgroup>
