{{-- Categories manager (AJAX-loaded inside category_pills) --}}
@php /** @var \App\Models\PageBlock $block */ @endphp

<section class="pt-4 mt-4 border-t border-gray-200">
    <div class="flex items-center justify-between mb-2">
        <h4 class="text-xs font-semibold uppercase tracking-wide text-gray-500">{{ __('admin.page_builder.config_forms.category_pills.categories') }}</h4>
    </div>
    ...
    <div class="relative mb-3">
        <input type="search"
            ...
            placeholder="{{ __('admin.page_builder.config_forms.category_pills.add_category') }}" data-action="search-block-categories" data-block-id="{{ $block?->id }}" />
    ...
    <div data-block-category-search-results data-block-id="{{ $block?->id }}"
    ...
    <div data-block-categories-list data-block-id="{{ $block?->id }}" class="space-y-1 text-sm text-gray-500">
