        ...
        <div class="col-span-12 lg:col-span-7 space-y-6">
            ...
            <x-card title="{{ __('admin.vendors.bank_account') }}">
                ...
                @if($primaryBank)
                    ...
                    @endif
                @else
                    <div class="flex items-center gap-2 rounded border border-dashed border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
                ...
                <ul class="space-y-2.5">
                    @foreach($checklist as $key => $item)
                        <li class="flex items-center gap-2.5">
                            ...
                            @elseif(($item['required'] ?? true) === false)
                                <span class="flex-shrink-0 w-5 h-5 rounded-full bg-amber-100 flex items-center justify-center">
                                    <svg class="w-3 h-3 text-amber-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.72-1.36 3.486 0l6.516 11.598c.75 1.334-.213 2.98-1.743 2.98H3.484c-1.53 0-2.492-1.646-1.743-2.98L8.257 3.1zM11 14a1 1 0 11-2 0 1 1 0 012 0zm-.25-6.25a.75.75 0 00-1.5 0v3.5a.75.75 0 001.5 0v-3.5z" clip-rule="evenodd"/></svg>
                                </span>
                                <span class="text-sm text-amber-600">{{ $item['label'] }} ({{ __('admin.vendor_applications.optional_for_payouts') }})</span>
                            @else
