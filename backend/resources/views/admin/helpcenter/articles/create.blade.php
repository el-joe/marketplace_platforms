...
@section('title', __('admin.helpcenter.new_article'))
...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ __('admin.helpcenter.new_article') }}</h1>
    ...
    <form id="article-form" method="POST" action="{{ route('admin.helpcenter.articles.store') }}">
        @csrf
                    ...
                    <div class="p-6 space-y-5">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="title_en" id="title-input" required maxlength="255"
                                       ...
                                       placeholder="{{ __('admin.helpcenter.title_placeholder') }}">
                        ...
                        <div>
                            ...
                            <input type="text" name="slug" id="slug-input" maxlength="255"
                                   ...
                                   placeholder="{{ __('admin.helpcenter.slug_auto_generated') }}">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <textarea name="excerpt_en" id="excerpt" rows="2" maxlength="500"
                                          ...
                                          placeholder="{{ __('admin.helpcenter.excerpt_placeholder') }}">{{ old('excerpt_en') }}</textarea>
                            ...
                            <select name="help_center_category_id" class="form-input w-full text-sm @error('help_center_category_id') is-invalid @enderror">
                                ...
                                @foreach($categories as $parent)
                                    <optgroup label="{{ $parent->name }}">
                                        <option value="{{ $parent->id }}" {{ old('help_center_category_id') == $parent->id ? 'selected' : '' }}>
                                            {{ $parent->name }}
                                        </option>
                                        @foreach($parent->children as $child)
                                            <option value="{{ $child->id }}" {{ old('help_center_category_id') == $child->id ? 'selected' : '' }}>
                                                ↳ {{ $child->name }}
                                            </option>
                                        @endforeach
                                    </optgroup>
                        ...
                        <div>
                            ...
                            <select name="country_id" class="form-input w-full text-sm">
                                ...
                                @foreach($countries as $c)
                                    <option value="{{ $c->site_code }}" {{ old('country_id') == $c->site_code ? 'selected' : '' }}>{{ $c->flag_emoji }} {{ $c->name_en }}</option>
                                @endforeach
                            </select>
                        </div>
