    ...
    <div id="app-shell" x-data="deliveryApp()" @availability-changed.window="isAvailable = $event.detail.is_available">
        ...
        @auth('delivery')
            ...
            @if($pendingCodSettlement)
                @php
                    $agent = auth('delivery')->user();
                    $codCurrency = $agent->country?->currency_code ?? $agent->zone?->country?->currency_code ?? 'SAR';
                    $isRtl = app()->getLocale() === 'ar';
                @endphp
                <div dir="{{ $isRtl ? 'rtl' : 'ltr' }}" class="bg-yellow-500/10 border-b border-yellow-500/30 px-4 py-3 {{ $isRtl ? 'text-right' : 'text-left' }}">
                    <a href="{{ route('delivery.cod-settlements.index') }}" class="flex items-start gap-2 text-yellow-300 text-sm leading-snug">
                        ...
                        <span>
                            {{ __('delivery.cod.settlement_due', ['amount' => number_format($pendingCodSettlement->net_to_remit, 2) . ' ' . $codCurrency]) }}
                        </span>
                    </a>
                </div>
            @endif
            ...
            <nav id="bottom-nav">
                <div class="flex h-16">
                    @php
                        ...
                        $navActive = fn (string $prefix, string $current): string
