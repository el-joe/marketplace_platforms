@extends('layouts.carrier')
@section('title', __('carrier.reports.claims_title'))
@section('content')

<div class="mb-4 flex items-center gap-1 border-b border-gray-200">
    <a href="{{ route('carrier.reports.orders') }}"
       class="px-4 py-2 text-sm font-medium {{ request()->routeIs('carrier.reports.orders*') ? 'text-indigo-600 font-bold border-b-2 border-indigo-600' : 'text-gray-500 hover:text-indigo-600' }}">
        {{ __('carrier.reports.tab_orders') }}
    </a>
    <a href="{{ route('carrier.reports.earnings') }}"
       class="px-4 py-2 text-sm font-medium {{ request()->routeIs('carrier.reports.earnings*') ? 'text-indigo-600 font-bold border-b-2 border-indigo-600' : 'text-gray-500 hover:text-indigo-600' }}">
        {{ __('carrier.reports.tab_earnings') }}
    </a>
    <a href="{{ route('carrier.reports.payouts') }}"
       class="px-4 py-2 text-sm font-medium {{ request()->routeIs('carrier.reports.payouts*') ? 'text-indigo-600 font-bold border-b-2 border-indigo-600' : 'text-gray-500 hover:text-indigo-600' }}">
        {{ __('carrier.reports.tab_payouts') }}
    </a>
    <a href="{{ route('carrier.reports.cod-settlements') }}"
       class="px-4 py-2 text-sm font-medium {{ request()->routeIs('carrier.reports.cod-settlements*') ? 'text-indigo-600 font-bold border-b-2 border-indigo-600' : 'text-gray-500 hover:text-indigo-600' }}">
        {{ __('carrier.reports.tab_cod_settlements') }}
    </a>
    <a href="{{ route('carrier.reports.performance') }}"
       class="px-4 py-2 text-sm font-medium {{ request()->routeIs('carrier.reports.performance*') ? 'text-indigo-600 font-bold border-b-2 border-indigo-600' : 'text-gray-500 hover:text-indigo-600' }}">
        {{ __('carrier.reports.tab_performance') }}
    </a>
    <a href="{{ route('carrier.reports.claims') }}"
       class="px-4 py-2 text-sm font-medium {{ request()->routeIs('carrier.reports.claims*') ? 'text-indigo-600 font-bold border-b-2 border-indigo-600' : 'text-gray-500 hover:text-indigo-600' }}">
        {{ __('carrier.reports.tab_claims') }}
    </a>
</div>

<div class="mb-6">
    <h1 class="text-2xl font-black text-gray-900">{{ __('carrier.reports.claims_title') }}</h1>
    <p class="text-sm text-gray-500 mt-0.5">{{ __('carrier.reports.claims_subtitle') }}</p>
</div>

{{-- Stats --}}
<div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
    <div class="bg-white rounded-xl border border-gray-200 p-4 text-center">
        <div class="text-2xl font-black text-gray-800">{{ $stats['total'] }}</div>
        <div class="text-xs text-gray-500 mt-1">{{ __('carrier.reports.claims_total') }}</div>
    </div>
    <div class="bg-white rounded-xl border border-amber-200 p-4 text-center">
        <div class="text-2xl font-black text-amber-600">{{ $stats['open'] }}</div>
        <div class="text-xs text-gray-500 mt-1">{{ __('carrier.reports.claims_open') }}</div>
    </div>
    <div class="bg-white rounded-xl border border-emerald-200 p-4 text-center">
        <div class="text-2xl font-black text-emerald-600">{{ $stats['approved'] }}</div>
        <div class="text-xs text-gray-500 mt-1">{{ __('carrier.reports.claims_approved') }}</div>
    </div>
    <div class="bg-white rounded-xl border border-red-200 p-4 text-center">
        <div class="text-xl font-black text-red-600">
            {{ number_format($stats['compensated']) }}
            <span class="text-sm font-medium text-gray-400">{{ $currency }}</span>
        </div>
        <div class="text-xs text-gray-500 mt-1">{{ __('carrier.reports.claims_compensated') }}</div>
    </div>
</div>

{{-- Filters --}}
<form method="GET" class="bg-white rounded-xl border border-gray-200 p-4 mb-4 flex flex-wrap gap-3 items-end">
    <div>
        <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('carrier.reports.filter_status') }}</label>
        <select name="status" class="border border-gray-300 rounded-lg px-3 py-2 text-sm">
            <option value="">{{ __('carrier.reports.all_statuses') }}</option>
            @foreach(['submitted','under_review','approved','rejected','compensated'] as $s)
                <option value="{{ $s }}" @selected(request('status') === $s)>{{ ucfirst(str_replace('_',' ',$s)) }}</option>
            @endforeach
        </select>
    </div>
    <div>
        <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('carrier.reports.claims_type') }}</label>
        <select name="claim_type" class="border border-gray-300 rounded-lg px-3 py-2 text-sm">
            <option value="">{{ __('carrier.reports.all_types') }}</option>
            @foreach(['lost','damaged','delayed','wrong_item','other'] as $t)
                <option value="{{ $t }}" @selected(request('claim_type') === $t)>{{ ucfirst(str_replace('_',' ',$t)) }}</option>
            @endforeach
        </select>
    </div>
    <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-4 py-2 rounded-lg text-sm transition">
        {{ __('carrier.reports.filter') }}
    </button>
    <a href="{{ route('carrier.reports.claims') }}" class="text-gray-500 text-sm hover:underline px-2 py-2">{{ __('carrier.reports.reset') }}</a>
</form>

{{-- Claims table --}}
<div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
    @if($claims->isEmpty())
    <div class="py-16 text-center text-gray-400 text-sm">{{ __('carrier.reports.no_data') }}</div>
    @else
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            <thead class="bg-gray-50 text-xs text-gray-500 uppercase">
                <tr>
                    <th class="px-4 py-3 text-left">{{ __('carrier.reports.col_claim_number') }}</th>
                    <th class="px-4 py-3 text-left">{{ __('carrier.reports.col_agent') }}</th>
                    <th class="px-4 py-3 text-left">{{ __('carrier.reports.col_tracking') }}</th>
                    <th class="px-4 py-3 text-left">{{ __('carrier.reports.claims_type') }}</th>
                    <th class="px-4 py-3 text-right">{{ __('carrier.reports.col_claimed') }} ({{ $currency }})</th>
                    <th class="px-4 py-3 text-right">{{ __('carrier.reports.col_compensated') }} ({{ $currency }})</th>
                    <th class="px-4 py-3 text-center">{{ __('carrier.reports.col_status') }}</th>
                    <th class="px-4 py-3 text-center"></th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
                @foreach($claims as $claim)
                @php
                    $sc = [
                        'submitted'    => 'amber',
                        'under_review' => 'blue',
                        'approved'     => 'emerald',
                        'compensated'  => 'emerald',
                        'rejected'     => 'red',
                    ][$claim->status->value] ?? 'gray';
                @endphp
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3 font-mono text-xs text-gray-600">{{ $claim->claim_number }}</td>
                    <td class="px-4 py-3 text-gray-800">{{ $claim->deliveryAgent?->name ?? '—' }}</td>
                    <td class="px-4 py-3 font-mono text-xs text-gray-500">{{ $claim->shipment?->tracking_number ?? '—' }}</td>
                    <td class="px-4 py-3 text-gray-600 capitalize">{{ str_replace('_', ' ', $claim->claim_type->value) }}</td>
                    <td class="px-4 py-3 text-right text-gray-700">{{ number_format($claim->claimed_amount) }}</td>
                    <td class="px-4 py-3 text-right {{ $claim->compensated_amount ? 'text-red-600 font-semibold' : 'text-gray-300' }}">
                        {{ $claim->compensated_amount ? number_format($claim->compensated_amount) : '—' }}
                    </td>
                    <td class="px-4 py-3 text-center">
                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-{{ $sc }}-100 text-{{ $sc }}-700 capitalize">
                            {{ str_replace('_', ' ', $claim->status->value) }}
                        </span>
                    </td>
                    <td class="px-4 py-3 text-center">
                        <a href="{{ route('carrier.reports.claims.show', $claim->id) }}"
                           class="text-indigo-600 hover:underline text-xs font-medium">
                            {{ __('carrier.common.view') }}
                        </a>
                    </td>
                </tr>
                @endforeach
            </tbody>
        </table>
    </div>
    <div class="px-4 py-3 border-t border-gray-100">{{ $claims->links() }}</div>
    @endif
</div>
@endsection
