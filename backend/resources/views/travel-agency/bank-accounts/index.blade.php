...
@section('title', __('travel.bank_accounts.title'))
@section('page-title', __('travel.bank_accounts.title'))

@push('scripts')
    @vite('resources/js/travel_agency/bank-accounts.js')
    <script>
        window.BANK_ACCOUNTS = {
            storeUrl:      '{{ route('travel-agency.bank-accounts.store') }}',
...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <p class="text-sm text-gray-500">{{ __('travel.bank_accounts.verified_only_note') }}</p>
        <button id="btn-add-account"
            ...
            {{ __('travel.bank_accounts.add_account') }}
        </button>
    </div>
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6 flex items-start gap-3">
        ...
        <p class="text-sm text-blue-700">
            {!! __('travel.bank_accounts.verification_notice', ['days' => '<strong>2</strong>']) !!}
        </p>
    </div>
    ...
    <div class="bg-yellow-50 border border-yellow-300 rounded-xl p-4 mb-6 flex items-start gap-3">
        <span>🔒</span>
        <p class="text-sm text-yellow-800">{{ __('travel.bank_accounts.section_locked_generic') }}</p>
    </div>

    @if ($pendingChangeRequests->isNotEmpty())
        <div class="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-6">
            <p class="text-sm text-gray-600 mb-1">{{ __('travel.bank_accounts.change_request_pending_notice') }}</p>
            <a href="{{ route('travel-agency.change-requests.index') }}" class="text-sm font-medium text-primary-600 hover:text-primary-800">
                {{ __('travel.bank_accounts.view_change_requests') }}
            </a>
        </div>
    @endif
    ...
    <div id="accounts-container">
        @if($accounts->isEmpty())
            <div id="empty-state" class="bg-white rounded-2xl border border-gray-200 py-16 text-center">
                ...
                <h3 class="font-semibold text-gray-800 mb-1">{{ __('travel.bank_accounts.no_accounts') }}</h3>
                <p class="text-sm text-gray-400 mb-4">{{ __('travel.bank_accounts.no_accounts_desc') }}</p>
        ...
        @else
            ...
            <div id="accounts-grid" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach($accounts as $account)
                    @php
                        $verificationMap = [
                            'pending'  => ['bg-yellow-100 text-yellow-700', __('travel.bank_accounts.verification.pending')],
                            'verified' => ['bg-green-100 text-green-700',   __('travel.bank_accounts.verification.verified')],
                            'rejected' => ['bg-red-100 text-red-700',       __('travel.bank_accounts.verification.rejected')],
                        ];
                    ...
                    <div class="bank-account-card bg-white rounded-2xl border {{ $account->is_primary ? 'border-yellow-400' : 'border-gray-200' }} p-5"
                            ...
                            <div class="flex items-center gap-1.5 shrink-0">
                                @if($account->is_primary)
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-700">
                                        {{ __('travel.bank_accounts.primary_badge') }} ★
                                    </span>
                                @endif
                        ...
                        <div class="space-y-1.5 text-sm mb-4">
                            <div class="flex justify-between">
                                <span class="text-gray-500">{{ __('travel.bank_accounts.account_holder') }}</span>
                                <span class="font-medium text-gray-800">{{ $account->account_holder_name }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500">{{ __('travel.bank_accounts.iban') }}</span>
                            ...
                            @endif
                            <div class="flex justify-between">
                                <span class="text-gray-500">{{ __('travel.bank_accounts.currency') }}</span>
                                <span class="font-medium text-gray-800">{{ $account->currency }}</span>
                            </div>
                        </div>

                        <div class="flex items-center gap-2 pt-3 border-t border-gray-50">
                            @if(!$account->is_primary && $account->verification_status === \App\Enums\TravelAgencyBankAccountVerificationStatus::Verified)
                                <button type="button"
                                        ...
                                        data-id="{{ $account->id }}">
                                    {{ __('travel.bank_accounts.set_primary') }}
                                </button>
                            @elseif($account->is_primary)
                                <span class="flex-1 text-xs text-center text-gray-400 py-2">{{ __('travel.bank_accounts.current_primary') }}</span>
                            @else
                            ...
                            <button type="button"
                                    ...
                                    data-id="{{ $account->id }}">
                                {{ __('travel.bank_accounts.delete') }}
                            </button>
                        </div>
                    </div>
                @endforeach
    ...
    <div id="add-account-modal" class="fixed inset-0 z-50 hidden items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div class="p-5 border-b border-gray-100 flex items-center justify-between">
                <h3 class="font-semibold text-gray-900">{{ __('travel.bank_accounts.add_new_account') }}</h3>
            ...
            <form id="add-account-form" class="p-5 space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">
                            {{ __('travel.bank_accounts.holder_name_label') }} <span class="text-red-500">*</span>
                        </label>
                        <input type="text" name="account_holder_name" required
                               class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400/40"
                               placeholder="{{ __('travel.bank_accounts.holder_placeholder') }}">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">
                            {{ __('travel.bank_accounts.bank_name_label') }} <span class="text-red-500">*</span>
                        </label>
                        <input type="text" name="bank_name" required
                               class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400/40"
                               placeholder="{{ __('travel.bank_accounts.bank_name_placeholder') }}">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('travel.bank_accounts.branch_label') }}</label>
                        <input type="text" name="branch"
                               class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400/40"
                               placeholder="{{ __('travel.bank_accounts.branch_placeholder') }}">
                    </div>
                    <div class="sm:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">
                        ...
                        <input type="text" name="iban" required maxlength="34" dir="ltr"
                               ...
                               placeholder="{{ __('travel.bank_accounts.iban_placeholder') }}">
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">
                            {{ __('travel.bank_accounts.currency') }} <span class="text-red-500">*</span>
                        </label>
                        <select name="currency" required
                                class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400/40">
                            <option value="">{{ __('travel.bank_accounts.currency_select') }}</option>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">{{ __('travel.bank_accounts.change_request_reason') }}</label>
                    <textarea name="note" rows="2"
                              class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400/40"></textarea>
                </div>
                ...
                <div class="flex gap-2 pt-1">
                    <button type="submit" id="account-submit-btn"
                        ...
                        {{ __('travel.bank_accounts.submit_account') }}
                    </button>
